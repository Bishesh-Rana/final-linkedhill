    ...
    public function postAgentLogin(Request $request){
        ...
        if($check->agentStaff != null){
            if($check->agentStaff->status == 'Not Verified'){
                return redirect()->back()->with('error', 'You are not verified. Please contact admin.');
            }elseif($check->agentStaff->status == 'Blocked'){
                return back()->with('error', 'You are blocked. Please contact to admin');
            }elseif($check->agentStaff->status == 'Rejected'){
                return back()->with('error', 'You are Rejected. Please contact to admin');
            }
